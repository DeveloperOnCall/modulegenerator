...
class ModuleGeneratorViewCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'modulegenerator:view
    ...
    public function __construct()
    {
        ...
        if (config('modulegenerator.view_columns_number')) {
            $this->defaultColumnsToShow = config('modulegenerator.view_columns_number');
        }

        $this->delimiter = config('modulegenerator.custom_delimiter')
            ? config('modulegenerator.custom_delimiter')
            : ['%%', '%%'];
    }
    ...
    public function handle()
    {
        ...
        $this->viewDirectoryPath = config('modulegenerator.custom_template')
            ? config('modulegenerator.path') . 'views/' . $formHelper . '/'
        ...
        $viewDirectory = app_path('Modules/'.ucfirst($this->modelName).'/Resources/Views/');
        ...
        $path = base_path('app/Modules/'.ucfirst($this->crudName).'/Resources/Views/');
        // dd($path);
        $this->viewTemplateDir = isset($this->userViewPath)
            ? $this->userViewPath . '.' . $this->viewName
            : $this->viewName;

        $this->viewTemplateDir = '';//app_path('Modules/'.$name.'/Resources/Views/');
        //$path = base_path('app/Modules/'.$name.'/Views/'); // config('view.paths')[0] . '/';
        //$this->viewTemplateDir = $path;
