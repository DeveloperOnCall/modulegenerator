...
namespace DeveloperOnCall\ModuleGenerator\Commands;
...
class ModuleGeneratorCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'modulegenerator:generate
    ...
    public function handle()
    {
        ...
        // generate the caf-module
        /* ModuleGenerator */
        Artisan::call('make:module', [
            'slug' => strtolower($name), //=>'cartalyst/sentry'
            '--quick' => true
        ]);
        /* ModuleGenerator */

        $this->call('modulegenerator:controller', ['name' => $controllerNamespace . $name . 'Controller', '--crud-name' => $name, '--model-name' => $modelName, '--model-namespace' => $modelNamespace, '--view-path' => $viewPath.'/Http/Controllers', '--route-group' => $routeGroup, '--pagination' => $perPage, '--fields' => $fields, '--validations' => $validations]);
        $this->call('modulegenerator:model', ['name' => $modelNamespace . $modelName, '--fillable' => $fillable, '--table' => $tableName, '--pk' => $primaryKey, '--relationships' => $relationships, '--soft-deletes' => $softDeletes]);
        $this->call('modulegenerator:migration', ['name' => $migrationName, '--schema' => $migrationFields, '--pk' => $primaryKey, '--indexes' => $indexes, '--foreign-keys' => $foreignKeys, '--soft-deletes' => $softDeletes]);
        $this->call('modulegenerator:view', ['name' => $name, '--fields' => $fields, '--validations' => $validations, '--view-path' => $viewPath, '--route-group' => $routeGroup, '--localize' => $localize, '--pk' => $primaryKey, '--form-helper' => $formHelper]);
        if ($localize == 'yes') {
            $this->call('modulegenerator:lang', ['name' => $name, '--fields' => $fields, '--locales' => $locales]);
        }
        ...
        $routeFile = app_path('Modules/'.$name.'/Routes/web.php');

        if (file_exists($routeFile) && (strtolower($this->option('route')) === 'yes')) {
            $this->controller = ($controllerNamespace != '') ? $controllerNamespace . '\\' . $name . 'Controller' : $name . 'Controller';
            $this->justController = $name . 'Controller';

            $isAdded = File::append($routeFile, "\n" . implode("\n", $this->addRoutes()));

            if ($isAdded) {
                $this->info('ModuleGenerator route added to ' . $routeFile);
            } else {
    ...
    protected function addRoutes()
    {
        $route = "
            ...
            Route::group(['prefix' => '" . $this->routeName . "'], function () {
                Route::resource('/', '".$this->justController."');
            });
